...
ruler.prototype.builder = function() {
    ...
    var constructCorner = (function() {
        function cornerDraw(container, side) {
            ...
            ruler.prototype.utils.addClasss(corner, [
                'rul_corner',
                cornerStyle
            ]);
            ...
            corner.style.height = ruler.prototype.utils.pixelize(
                options.rulerHeight
            );
    ...
    var constructRulers = function(curOptions) {
        theRulerDOM = ruler.prototype.utils.addClasss(
            theRulerDOM,
            'rul_wrapper'
        );
    ...
    var draggable = (function() {
        return {
            ...
            startMoving: function(evt) {
                ruler.prototype.utils.addClasss(guideLine, [
                    'rul_line_dragged'
                ]);
            ...
            stopMoving: function() {
                ...
                hideToolTip();
                ruler.prototype.utils.removeClasss(guideLine, [
                    'rul_line_dragged'
                ]);
            }
        };
    ...
    var updateToolTip = function(x, y) {
        if (y) {
            guideLine.dataset.tip =
                'Y: ' +
                Math.round(
                    (y - options.rulerHeight - 1 - _curPosDelta) * _curScale
                ) +
                ' px';
        } else {
            guideLine.dataset.tip =
                'X: ' +
                Math.round(
                    (x - options.rulerHeight - 1 - _curPosDelta) * _curScale
                ) +
                ' px';
        }
    };
